...
const Details = () => {
    ...
    useEffect(() => {
        ...
        if (pwhatis === "pokemon") {
            userServices.getPokemon(pname).then(async result => {
                setData(result)
                const abilitiesEN = result.abilities.map(e => e.ability.url)

                const description = await userServices.getDescriptionPokemon(pname)
                setDescription(description)

                //obiente habilidades en español
                const abilitiesEs = await Promise.all(
                    abilitiesEN.map(async (url) => {
                        const abilityData = await userServices.getAbilitiesES(url)
                ...
                const typesRes = await Promise.all(
                    result.types.map(async (type) => {
                        return await userServices.getTypesES(type.type.url)
                    })
                )
                setTypes(typesRes)
                ...
                const statsRes = await Promise.all(
                    result.stats.map(async (stat) => {
                        return { name: await userServices.getStatsES(stat.stat.url), value: stat.base_stat }
                    })
                )
                setStats(statsRes)
                ...
                const moveRes = await Promise.all(
                    result.moves.map(async (move) => {
                        return await userServices.getMoveES(move.move.url)
                    })
                )
                setMoves(moveRes)
                ...
                const specieData = await userServices.getDataUrl(result.species.url)
                const evolutionUrl = specieData.evolution_chain.url
                const evolutionData = await userServices.getDataUrl(evolutionUrl)
        ...
        if (pwhatis === "items") {
            userServices.getItem(pname).then(async result => {
                setData(result)
                console.log(result)
            })
        ...
        const getDataEvolution = async () => {
            ...
            const evolution = await Promise.all(
                dataEvolution.map(async (name) => {
                    const data = await userServices.getPokemon(name);
                    return {
                        name: data.name,
