...
export function StoreProvider({ children }) {
  ...
  useEffect(() => {
    userServices.getPokemons().then(data => {

      dispatch({ type: 'getPokemons', payload: { data: data.results } });


      data.results.forEach(async (pokemon) => {
        const infoPokemon = await userServices.getPokemon(pokemon.name);
    ...
    userServices.getItems().then(data => {
      dispatch({ type: 'getItems', payload: { data: data.results } });

      data.results.forEach(async (item) => {
        const infoItem = await userServices.getItem(item.name);
